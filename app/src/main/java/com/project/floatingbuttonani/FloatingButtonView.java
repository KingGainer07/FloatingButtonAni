...
package com.project.floatingbuttonani;
...
import static com.project.floatingbuttonani.FloatingButtonActionItem.RESOURCE_NOT_SET;
import static java.lang.annotation.RetentionPolicy.SOURCE;

@SuppressWarnings({"unused", "WeakerAccess", "UnusedReturnValue"})
public class FloatingButtonView extends LinearLayout implements CoordinatorLayout.AttachedBehavior {
    ...
    @Nullable
    private FloatingButtonOverlayLayout mOverlayLayout;
    @Nullable
    ...
    private OnActionSelectedListener mOnActionSelectedProxyListener = new OnActionSelectedListener() {
        @Override
        public boolean onActionSelected(FloatingButtonActionItem actionItem) {
    ...
    public FloatingButtonView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void setExpansionMode(@ExpansionMode int expansionMode, boolean force) {
        if (mInstanceState.mExpansionMode != expansionMode || force) {
            ...
            close(false);
            ArrayList<FloatingButtonActionItem> actionItems = getActionItems();
            clearActionItems();
    ...
    @Nullable
    public FloatingButtonOverlayLayout getOverlayLayout() {
     ...
     */
    public void setOverlayLayout(@Nullable FloatingButtonOverlayLayout overlayLayout) {
    ...
    public void inflate(@MenuRes int menuRes) {
        ...
        for (int i = 0; i < menu.size(); i++) {
            ...
            FloatingButtonActionItem actionItem = new FloatingButtonActionItem.Builder(menuItem.getItemId(), menuItem.getIcon())
    ...
    /**
     * Appends all of the {@link FloatingButtonActionItem} to the end of the list, in the order that they are returned by
     * the specified
     * collection's Iterator.
     *
     * @param actionItemCollection collection containing {@link FloatingButtonActionItem} to be added to this list
    ...
    /**
     * Inserts the specified {@link FloatingButtonActionItem} at the specified position in this list. Shifts the element
     * currently at that position (if any) and any subsequent elements to the right (adds one to their indices).
     *
     * @param actionItem {@link FloatingButtonActionItem} to be appended to this list
     * @param position   index at which the specified element is to be inserted
     * @return the instance of the {@link FabWithLabelView} if the add was successful, null otherwise.
     */
    @Nullable
    public FabWithLabelView addActionItem(FloatingButtonActionItem actionItem, int position) {
    ...
    /**
     * Removes the {@link FloatingButtonActionItem} at the specified position in this list. Shifts any subsequent elements
     * to the left (subtracts one from their indices).
     *
     * @param position the index of the {@link FloatingButtonActionItem} to be removed
     * @return the {@link FloatingButtonActionItem} that was removed from the list
     */
    @Nullable
    ...
    /**
     * Removes the specified {@link FloatingButtonActionItem} from this list, if it is present. If the list does not
     * contain the element, it is unchanged.
     * <p>
     * Returns true if this list contained the specified element (or equivalently, if this list changed
     * as a result of the call).
     *
     * @param actionItem {@link FloatingButtonActionItem} to be removed from this list, if present
     * @return true if this list contained the specified element
     */
    public boolean removeActionItem(@Nullable FloatingButtonActionItem actionItem) {
        return actionItem != null && removeActionItemById(actionItem.getId()) != null;
    }

    /**
     * Finds and removes the first {@link FloatingButtonActionItem} with the given ID, if it is present. If the list does not
     * contain the element, it is unchanged.
     *
     * @param idRes the ID to search for
     * @return the {@link FloatingButtonActionItem} that was removed from the list, or null otherwise
     */
    @Nullable
    public FloatingButtonActionItem removeActionItemById(@IdRes int idRes) {
        return removeActionItem(findFabWithLabelViewById(idRes));
    }

    /**
     * Replace the {@link FloatingButtonActionItem} at the specified position in this list with the one provided as
     * parameter.
     *
     * @param newActionItem {@link FloatingButtonActionItem} to use for the replacement
     * @param position      the index of the {@link FloatingButtonActionItem} to be replaced
    ...
    /**
     * Removes all of the {@link FloatingButtonActionItem} from this list.
     */
    ...
    protected void onAttachedToWindow() {
        ...
        if (mOverlayLayout == null) {
            FloatingButtonOverlayLayout overlayLayout = getRootView().findViewById(mOverlayLayoutId);
    ...
    @Nullable
    private FloatingButtonActionItem removeActionItem(@Nullable FabWithLabelView view,
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            setElevation(getResources().getDimension(R.dimen.close_elevation));
        }
        TypedArray styledAttrs = context.obtainStyledAttributes(attrs, R.styleable.FloatingView, 0, 0);
        try {
            setEnabled(styledAttrs.getBoolean(R.styleable.FloatingView_android_enabled, isEnabled()));
            setUseReverseAnimationOnClose(styledAttrs.getBoolean(R.styleable.FloatingView_sdUseReverseAnimationOnClose,
                    getUseReverseAnimationOnClose()));

            setMainFabAnimationRotateAngle(styledAttrs.getFloat(R.styleable.FloatingView_sdMainFabAnimationRotateAngle,
                    getMainFabAnimationRotateAngle()));
            @DrawableRes int openDrawableRes = styledAttrs.getResourceId(R.styleable.FloatingView_sdMainFabClosedSrc,
                    RESOURCE_NOT_SET);
            if (openDrawableRes != RESOURCE_NOT_SET) {
                setMainFabClosedDrawable(AppCompatResources.getDrawable(getContext(), openDrawableRes));
            }
            int closeDrawableRes = styledAttrs.getResourceId(R.styleable.FloatingView_sdMainFabOpenedSrc,
                    RESOURCE_NOT_SET);
            if (closeDrawableRes != RESOURCE_NOT_SET) {
                setMainFabOpenedDrawable(AppCompatResources.getDrawable(context, closeDrawableRes));
            }
            setExpansionMode(styledAttrs.getInt(R.styleable.FloatingView_sdExpansionMode, getExpansionMode()), true);

            setMainFabClosedBackgroundColor(styledAttrs.getColor(R.styleable
                            .FloatingView_sdMainFabClosedBackgroundColor,
                    getMainFabClosedBackgroundColor()));
            setMainFabOpenedBackgroundColor(styledAttrs.getColor(R.styleable
                            .FloatingView_sdMainFabOpenedBackgroundColor,
                    getMainFabOpenedBackgroundColor()));
            setMainFabClosedIconColor(styledAttrs.getColor(R.styleable
                            .FloatingView_sdMainFabClosedIconColor,
                    getMainFabClosedIconColor()));
            setMainFabOpenedIconColor(styledAttrs.getColor(R.styleable
                            .FloatingView_sdMainFabOpenedIconColor,
                    getMainFabOpenedIconColor()));
            mOverlayLayoutId = styledAttrs.getResourceId(R.styleable.FloatingView_sdOverlayLayout, RESOURCE_NOT_SET);
        } catch (Exception e) {
    ...
    private FloatingActionButton createMainFab() {
        ...
        floatingActionButton.setId(R.id.main_fab);
    ...
    private void updateElevation() {
        if (isOpen()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                setElevation(getResources().getDimension(R.dimen.open_elevation));
            } else {
                bringToFront();
            }
        } else {
    ...
    private void showWithAnimationFabWithLabelView(FabWithLabelView fabWithLabelView, int delay) {
        ...
        if (fabWithLabelView.isLabelEnabled()) {
            ...
            Animation animation = AnimationUtils.loadAnimation(getContext(), R.anim.fade_and_translate_in);
    ...
    private void hideWithAnimationFabWithLabelView(final FabWithLabelView fabWithLabelView, int delay) {
        ...
        if (fabWithLabelView.isLabelEnabled()) {
            ...
            ViewCompat.animate(labelBackground).cancel();
            Animation animation = AnimationUtils.loadAnimation(getContext(), R.anim.fade_and_translate_out);
    ...
    public interface OnActionSelectedListener {
         ...
         *
         * @param actionItem the {@link FloatingButtonActionItem} that was selected.
         * @return true to keep the Speed Dial open, false otherwise.
         */
        boolean onActionSelected(FloatingButtonActionItem actionItem);
    }
    ...
    public static class SnackbarBehavior extends CoordinatorLayout.Behavior<View> {
        ...
        protected void show(View child) {
            ...
            } else if (child instanceof FloatingButtonView) {
                ((FloatingButtonView) child).show(mInternalAutoHideListener);
            } else {
        ...
        protected void hide(View child) {
            if (child instanceof FloatingActionButton) {
                ...
                ((FloatingButtonView) child).hide(mInternalAutoHideListener);
            } else {
